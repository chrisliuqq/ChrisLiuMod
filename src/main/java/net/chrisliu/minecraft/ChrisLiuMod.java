...
@Mod(modid = ChrisLiuMod.MODID, name = ChrisLiuMod.MODID, version = "1.7.2-1.1.1")
public final class ChrisLiuMod
{
	public static final String MODID = "ChrisLiuMod";
	
	public static Logger gbLog = Logger.getLogger("ChrisLiuMod");
	...
	/** Custom GUI indices: */
	public static final int GUI_TELEPORT = modGuiIndex++;
	...
	public static boolean ENABLE_PLAYER_TELEPORT;
	
	@EventHandler
	public void preInit(FMLPreInitializationEvent event) {
		Configuration config = new Configuration(new File(event.getModConfigurationDirectory().getAbsolutePath() + "/ChrisLiuMod.cfg"));
		config.load();
		//wizardArmorFlag = config.get(Configuration.CATEGORY_GENERAL, "WizardArmorFlag", true).getBoolean(true);
		CONFIG_URL = config.get(Configuration.CATEGORY_GENERAL, "HomeLocationConfigURL", "https://dl.dropboxusercontent.com/u/209570/Minecraft/ChrisLiuMod_config.txt").getString();
		ENABLE_PLAYER_TELEPORT = config.get(Configuration.CATEGORY_GENERAL, "EnablePlayerTeleport", false).getBoolean(true);
		config.save();
	}
